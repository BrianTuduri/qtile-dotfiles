...
def gradleVersion
def gradleCommands
def gradleFileExtension
def gradleParams = ""
def isRegenerate = params.tarea == "regenerate_version"
def localCheckout = isRegenerate ? params.version : params.branch
def toCheckout = (isRegenerate ? "refs/tags/${projectName}-${params.version}" : "*/${params.branch}")
def URL_REPO = params.service_repo_url
def projectName = params.proyecto.split('/')[-1]
def path_build = projectName == "geohub" ? "backend/" : ""
//Hash map con los servicios que necesitan NodeJS y ademas en el valor esta la version del NodeJS
def projects_NodeJS = [ 'geohub' : "8.11.3", 'digi-services' : "6.3.0", 'geopos-audit-log-service' : "6.3.0", 'alkosto-aws-mediator' : "6.3.0"]
def nodeVersion = projects_NodeJS["${projectName}"]
def tmpBuildVolume
def tmpJenkinsVolume
def gradleArgs = ["--no-daemon"].join(" ")
...
pipeline {
    ...
    environment {
        ...
        JENKINS_TMP_BUILDS_VOLUME = "/tmp/builds" // directory mapped to the container that jenkins can access
    }
    stages {
        stage('Checkout . . .') {
            steps {
            ...
            steps {
                script{
                    dir('SCM'){
                        ...
                        gradleFileExtension = varYml["${projectName}"]["extension"]
                        gradleParams = varYml["${projectName}"]["gradleParams"]
                        gradleCommands = getGradleCommands(params.tarea, gradleParams)
                        println ("\nExtension: ${gradleFileExtension}\n")
            ...
            steps {
                script {
                    dir(CONFIG["CHECKOUT_FOLDER"]) {
                        def CURRENT_BUILD_VOLUME = "/slave/.tmp_builds/tmp/:${IMAGE_WORKDIR}/"
                            def sshVolume
                            withDockerEnvironment(CONFIG, 
                                ...
                                { containerId -> // exec in docker
                                    ...
                                    shDocker "mkdir -p /root/.ssh/ ${tmpBuildVolume} && chown -R 1000:1000 ${tmpBuildVolume}"
                                    
                                    // set gradle.properties
                                    configFileProvider([configFile(fileId: '65be0b0c-afab-4f50-aad4-dcfa4ac63915', targetLocation: "${tmpJenkinsVolume}/gradle.properties")]) {
                                        sh "docker cp ${tmpJenkinsVolume}/gradle.properties ${containerId}:/root/.gradle/gradle.properties" // Copy the SSH key into the container
                                    }
                                    ...
                                    shDocker """
                                        source /usr/local/nvm/nvm.sh && nvm use ${nodeVersion}
                                        cd ${tmpBuildVolume}/${projectName} && ${gradleCommands} ${gradleArgs}
                                        chown -R 1000:1000 ${tmpBuildVolume}
                                    """
                                }
                            )
                ...
                script{
                    dir("${tmpJenkinsVolume}/${projectName}/${path_build}build/libs"){
                        if (params.tarea != "build") {
                            sh "mkdir ${projectName}-${gradleVersion}"
                            sh "cp ${projectName}-${gradleVersion}.jar ${projectName}-${gradleVersion}"
                            zip zipFile: "${projectName}-${gradleVersion}.zip", archive: false, dir: "${projectName}-${gradleVersion}"
                        }
                    }
                }
            }
        ...
        stage('Publicando zip en Nexus . . .') {
            ...
            steps {
                script {
                    def file = "${projectName}-${gradleVersion}.${gradleFileExtension}"
                    println "Version-- [${gradleVersion}]--"
                    println "Archivo para deploy Nexus -- [${file}]--"

                    dir("${tmpJenkinsVolume}/${projectName}/${path_build}build/libs") {
                        nexusArtifactUploader artifacts: [[artifactId: "${projectName}", classifier: '', file: file, type: gradleFileExtension]],
...
def getGradleCommands(String task, String gradleParams) {
    switch (task) {
        case "release":
                ...
                return "./gradlew release ${gradleParams}"
