        ...
        stage('Get plugins . . .') {
            ...
            steps {
                script {
                    dir('client') {
                        // try {
                        def pluginsDir = new File("${pwd()}/plugins")
                        if (!pluginsDir.exists()) {
                            pluginsDir.mkdirs()
                        }
                        def urls = params.FILE_URLS.trim().split(",")
                        urls.each { url ->
                            // Crear un nuevo File para el plugin específico a descargar.
                            def pluginFile = new File(pluginsDir, url.split("/").last())
                            println("Downloading plugin to: ${pluginFile.path}")
                            def processBuilder = new ProcessBuilder(['curl', '-s', '-o', pluginFile.path, url])
                        ...
                        dir('client') {
                            try {
                                ...
                                urls.each { url ->
                                    ...
                                    println("curl -s -o ${pluginsDir}/${archivo} ${url}")
