...
resource "vault_generic_secret" "environment" {
  ...
  for_each   = var.teams
  path       = "${vault_mount.kv_v2[each.key].path}/qa"
...
resource "vault_generic_secret" "qa-secrets" {
  depends_on = [vault_mount.kv_v2, vault_generic_secret.environment]
  for_each   = var.teams
  path       = "${vault_mount.kv_v2[each.key].path}/data/qa/do-not-use-me"
  data_json = jsonencode({
    placeholder = "This is a placeholder to create the QA folder."
  })
}
