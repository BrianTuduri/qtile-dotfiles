...
pipeline {
    ...
    stages {
        ...
        stage(' Establecer conexión con la VPN . . .') {
            ...
            steps {
                script {
                    if (!params.vpn_host.trim() || !params.vpn_port.trim() || !params.vpn_credentials.trim() || !params.registry_remote_credentials.trim()) {
                        error("Cuando se utiliza VPN, los campos [vpn_host, vpn_port, vpn_credentials, registry_remote_credentials] deben estar completos.")
                    }
        ...
        stage('Setup environment variables . . .') {
            steps {
                script {
                    ...
                    newCompleteUrl = result["newRegistryUrl"]
                ...
                script {
                    def getGitlabConfig = {
                        withCredentials([usernamePassword(credentialsId: params.registry_remote_credentials, usernameVariable: 'REGISTRY_USERNAME', passwordVariable: 'REGISTRY_TOKEN')]) {
                            ...
                            private_token=${env.REGISTRY_TOKEN}
                            """
                        }
                    }
                    String configText = getGitlabConfig()
                    writeFile file: '/etc/python-gitlab.cfg', text: configText
                }
            }
        }
        ...
        stage('Bajar imagen registry de origen y crear tag [LOCAL]...') {
            steps {
                script {
                    ...
                    usernamePassword(credentialsId: params.registry_local_credentials, passwordVariable: 'GITLAB_TOKEN', usernameVariable: 'GITLAB_USER')]) {                       docker.withTool('docker') {
                            docker.withServer(IP_PORT, CONFIG["DOCKER_SERVER_CREDENTIAL"]) {
                                docker.withRegistry("https://${baseRegistryUrl}", registry_local_credentials) {
                                    dockerLogin("https://${baseRegistryUrl}", GITLAB_USER, GITLAB_TOKEN)
                                    dockerPull("${params.registry_url_local}")
                                    dockerTag("${params.registry_url_local}", "${newCompleteUrl}")
                                    dockerLogout("https://${baseRegistryUrl}")
                                }
                            }
                        }
                    }
            ...
            steps {
                script {
                    ...
                    withCredentials([usernamePassword(credentialsId: params.registry_remote_credentials, usernameVariable: 'GITLAB_USER', passwordVariable: 'GITLAB_TOKEN')]) {
                        def outPut = executeCommand("python3 gitlab_management.py '${newGitLabBaseDomain}' '${GITLAB_TOKEN}' '${newGitLabPath}'")
                    }
                    def baseNewUrl = newRegistryUrl.split('/')[0]
                    println("Remote base url: ${baseNewUrl}")
                    withCredentials([usernameColonPassword(credentialsId: CONFIG["DOCKER_IP_CREDENTIAL"], variable: 'IP_PORT'),
                    usernamePassword(credentialsId: params.registry_remote_credentials, passwordVariable: 'GITLAB_TOKEN', usernameVariable: 'GITLAB_USER')]) {
                        docker.withTool('docker') {
                            docker.withServer(IP_PORT, CONFIG["DOCKER_SERVER_CREDENTIAL"]) {
                                docker.withRegistry("https://${baseNewUrl}", params.registry_remote_credentials) {
                                    dockerLogin("https://${baseNewUrl}", GITLAB_USER, GITLAB_TOKEN)
...
def dockerPush(String imageUrl) {
    executeDockerCommand("push ${imageUrl}")
}
