...
pipeline {
    ...
    parameters {
        ...
        string(name: 'gitlab_remote_repo_url', defaultValue: 'https://git-pivot.geocom.com.uy/uy-com-geocom-geosalud/geosalud-registry.git', description: 'URL del proyecto en Gitlab (REMOTO) Ejemplo: https://git-pivot.geocom.com.uy/uy-com-geocom-geosalud/geosalud-registry.git') // local & requerido
        ...
        stage('Subiendo imagen a registry remoto docker [REMOTO]. . .') {
            steps {
                script {
                    ...
                    withCredentials([usernamePassword(credentialsId: params.REGISTRY_LOCAL_CREDENTIALS, usernameVariable: 'GITLAB_USER', passwordVariable: 'GITLAB_TOKEN')]) {
                        def outPut = executeCommand("python3 test/gitlab_management.py '${parREMOTE_URLams.}' '${GITLAB_TOKEN}' '${pathRepository}'")
                    }
