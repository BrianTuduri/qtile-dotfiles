...
pipeline {
    ...
    parameters {
        // registry
        string(name: 'registry_url_local', defaultValue: '', description: 'URL del repositorio de imagenes (LOCAL) Ejemplo: registry.gitlab.geocom.com.uy:5005') // local & requerido
        string(name: 'registry_local_credentials', defaultValue: '', description: 'ID de credenciales en JENKINS de la VPN. (LOCAL) (En caso de que se necesite).') // VPN
        string(name: 'registry_url_remote', defaultValue: '', description: 'URL del repositorio de imagenes (REMOTO)') // local & requerido
        string(name: 'registry_remote_credentials', defaultValue: '', description: 'ID de credenciales en JENKINS de la VPN. (REMOTO) (En caso de que se necesite).') // VPN
    ...
    stages {
        ...
        stage('Config connection to Gitlab. . .'){
            steps{
                script {
                    withCredentials([usernamePassword(credentialsId: params.REGISTRY_CREDENTIALS, usernameVariable: 'REGISTRY_USERNAME', passwordVariable: 'REGISTRY_TOKEN')]) {
                        def getGitlabConfig = {
                            return """
                            [global]
                            default=geocom
        ...
        stage('Bajar imagen registry de origen y crear tag [LOCAL]...') {
            steps {
                script {
                    def urlClient = params.registry_url_local.replace("registry.gitlab.geocom.com.uy:5005", "${params.registry_url_remote}")

                    withCredentials([usernamePassword(credentialsId: 'a8bd70c2-566d-475c-9868-e87b42a3827b', passwordVariable: 'REGISTRY_PASS', usernameVariable: 'REGISTRY_USER')]) {
                        docker.withTool('docker') {
                            docker.withServer("${CONFIG["DOCKER_IP_CREDENTIAL"]}", 'Docker_Certs') {
                                docker.withRegistry('http://registry.gitlab.geocom.com.uy:5005', 'jenkins_registry_user') {
                                    dockerLogin('registry.gitlab.geocom.com.uy:5005', REGISTRY_USER, REGISTRY_PASS)
                                    dockerPull("${params.registry_url_local}:${params.version}")
                                    dockerTag("${params.registry_url_local}:${params.version}", "${urlClient}:${params.version}")
                                    executeDockerCommand("images")
                                    dockerLogout('http://registry.gitlab.geocom.com.uy:5005')
                                }
                            }
                        }
                    }
        ...
        stage('Subiendo imagen a registry remoto docker [REMOTO]. . .') {
            steps {
                script {
                    def projectName = "${params.registry_url_local.split("/").last()}"
                    def namespaceName = "${params.registry_url_local.split("/")[1]}"
                    def namespaceID
                    ...
                    //check if no existe n
                    /*
                    python gitlab_script.py -NN $namespaceName         devuelve el ID del proyecto
                    python gitlab_script.py -P YourProjectName          devuelve si existe el proyecto
                    python gitlab_script.py -N YourNamespaceName        devuelve si existe el namespace
                     */
                    def outPut = executeCommand("python scripts/gitlab_scriptP.py -P ${projectName}/${namespaceName}") == "False" ? false : true

                    if(!outPut) {//verifico si existe el proyecto

                        outPut = executeCommand("python scripts/gitlab_scriptP.py -N ${namespaceName}") == "False" ? false : true
                        if(!outPut){//verifico si existe el namespace
                            sh "gitlab group create -h --name ${namespaceName} --path ${namespaceName}"
                        }
                        namespaceID = executeCommand("python scripts/gitlab_scriptP.py -NN ${namespaceName}")
                        sh "gitlab create ${projectName} --name --namespace-id ${namespaceID}"
                    }
                    urlClient=params.registry_url_local.replace("registry.gitlab.geocom.com.uy:5005","${params.registry_url_remote}")
                    docker.withTool('docker'){
