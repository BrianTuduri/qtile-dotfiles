    ...
    stages {
        ...
        stage('Read and Merge Configuration . . .') {
            steps {
                script {
                    // set base config.yml
                    configFile = readYaml file: CONFIG["CONFIG_PATH_YML"]
                    ...
                    if (env.PATH_OVERRIDE_CONFIG != null) {
                        ...
                        configFile = mergeConfigs(configFile, overrideConfig)
                    }

                    def tmpMap = [
                        DOCKER_IP_CREDENTIAL: configFile.jenkins.credentials.docker_ip_credential,
                        DOCKER_SERVER_CREDENTIAL: configFile.jenkins.credentials.docker_server_credential,
                        NEXUS_REPOSITORY: configFile.nexus.credentials.repository,
                        NEXUS_GROUP_ID: configFile.nexus.credentials.group_id,
                        NEXUS_CREDENTIAL_ID: configFile.nexus.credentials.credential_id,
                        GIT_CREDENTIAL_ID: configFile.jenkins.credentials.git_credential_id,
                        DOCKER_IMAGE: configFile.docker.image,
                        DOCKER_ARGS: configFile.docker.args.join(' '),
                        IMAGE_WORKDIR: configFile.docker.workdir,
                        VOLUMES: configFile.docker.volumes.join(' -v '),
                        MAVEN_CONFIG_ID: configFile.jenkins.files.maven.config_file_id.toString(),
                        MAVEN_TARGET_DIR: configFile.jenkins.files.maven.target_location.toString(),
                        GRADLE_CONFIG_ID: configFile.jenkins.files.gradle.config_file_id.toString(),
                        GRADLE_TARGET_DIR: configFile.jenkins.files.gradle.target_location.toString()
                    ]
                ...
                script {
                    dir(CONFIG["CHECKOUT_FOLDER"]) {
                        ...
                        Closure dockerExec = { containerId ->
                            ...
                            // Basic setup within the container
                            ...
                            dir("${env.TMP_JENKINS_VOLUME}/${projectName}") {
                                // Define variables to determine the project type
                                mavenProject = fileExists('pom.xml') && !fileExists('build.gradle')
                                gradleProject = fileExists('build.gradle') && !fileExists('pom.xml')

                                // Decide build tool and version based on project type
                                if (mavenProject) {
                                    buildVersion = readMavenPom(file: 'pom.xml').getVersion().replaceAll("-SNAPSHOT", "")
                                    buildTool = wrapper ? "./mvnw" : "mvn"
                                    echo 'Proyecto Maven'
                                } else if (gradleProject) {
                                    properties = readProperties file: 'gradle.properties'
                                    buildVersion = properties['version'].replaceAll('-SNAPSHOT', '')
                                    buildTool = wrapper ? "./gradlew" : "gradle"
                                    echo 'Proyecto Gradle'
                                }
                                
                                // Run configFileProvider at the appropriate level
                                configFileProvider([configFile(fileId: gradleProject ? CONFIG["GRADLE_CONFIG_ID"] : CONFIG["MAVEN_CONFIG_ID"], targetLocation: gradleProject ? CONFIG["GRADLE_TARGET_DIR"] : CONFIG["MAVEN_TARGET_DIR"])]) {
                                    if (gradleProject) {
                                        sh "docker cp ${env.HOME}/.m2/gradle.properties ${containerId}:/root/.gradle/gradle.properties"
                                    }
                                }
                            }
                        }
...
def withDockerEnvironment(config, Closure dockerRunConfig, Closure body, String tmpBuildVolume) {
        ...
        docker.withTool('docker') {
            docker.withServer(IP_PORT, 'Docker_Certs') {
                ...
                try {
                    ...
                    if (containerId) {
                        stopAndRemoveDockerContainer(containerId, tmpBuildVolume)
...
def stopAndRemoveDockerContainer(String containerId, String tmpBuildVolume) {
    def shDocker = { command -> sh "docker exec ${containerId} sh -c '${command}'" }
    shDocker "chown -R 1000:1000 ${tmpBuildVolume}"
