            ...
            steps {
                script {
                    ...
                    def tmpMap = [
                        ...
                        MAVEN_CONFIG_ID: configFile.jenkins.files.maven.config_file_id,
                        MAVEN_TARGET_DIR: configFile.jenkins.files.maven.target_location,
                        GRADLE_CONFIG_ID: configFile.jenkins.files.gradle.config_file_id,
                        GRADLE_TARGET_DIR: configFile.jenkins.files.gradle.target_location
                    ]
                    ...
                    dir(CONFIG["CHECKOUT_FOLDER"]) {
                        ...
                        Closure dockerExec = { containerId -> // exec in docker
                            ...
                            dir("${env.TMP_JENKINS_VOLUME}/${projectName}") {
                                ...
                                configFileProvider([configFile(fileId: mavenProject ? CONFIG["MAVEN_CONFIG_ID"].toString() : CONFIG["GRADLE_CONFIG_ID"].toString(),
                                    targetLocation: mavenProject ? CONFIG["MAVEN_TARGET_DIR"].toString() : CONFIG["GRADLE_TARGET_DIR"].toString())] ) {
                                    
                                    if (gradleProject) {
                                        sh "docker cp ${env.TMP_JENKINS_VOLUME}/gradle.properties ${containerId}:/root/.gradle/gradle.properties" // Copy the SSH key into the container
                                    }
                                }
                            }
