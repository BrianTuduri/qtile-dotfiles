                ...
                script {
                    ...
                    try {
                        dir ('repository') { 
                            ...
                            if (!fileExists(file: env.PATH_DEFINED_PARAMETERS)) {
                                error("File: ${env.PATH_DEFINED_PARAMETERS} not exist")
                            }
        ...
        docker.withTool('docker') {
            docker.withServer(IP_PORT, 'Docker_Certs') {
                ...
                try {
                    ...
                    if (containerId) {
                        stopAndRemoveDockerContainer(containerId, tmpBuildVolme, CONFIG["USER_FOR_BUILD"], CONFIG["GROUP_FOR_BUILD"])
...
def stopAndRemoveDockerContainer(String containerId, String tmpBuildVolme, String id_user_for_build, String id_group_for_build) {
    def shDocker = { command -> sh "docker exec ${containerId} sh -c '${command}'" }
    shDocker "chown -R ${id_user_for_build}:${id_group_for_build} ${tmpBuildVolme}"
