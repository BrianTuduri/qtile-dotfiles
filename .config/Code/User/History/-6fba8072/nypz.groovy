...
// is maven or gradle
def (mavenProject, gradleProject) = [false, false]
...
pipeline {
    ...
    stages {
        ...
        stage('Set vars . . .') {
            steps {
                script {
                    (mavenProject, gradleProject) = initializeProjectType(params)
                    ...
                    configFileProvider([configFile(fileId: 'af5f8d3f-79e7-4ed3-8241-c0b831982733', targetLocation: 'settings.xml')]) {
                        echo "hola hola"
                    }
                }
            }
...
def initializeProjectType(params) {
    if(params.tool == null) {
        error("Set the hidden tool parameter with value: 'maven' or 'gradle'")
    } else {
        switch (params.tool.toLowerCase()) {
            ...
            case "gradle":
                return [false, true]
            default:
                error("Invalid tool parameter. Please specify 'maven' or 'gradle'.")
        }
    }
}