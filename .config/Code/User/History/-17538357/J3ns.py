...
# Listar los repositorios en el path especificado
try:
    list_cmd = ["for", "d", "in", REPO_PATH, ";", "do", "basename", "\"$d\"",";" "done"]
    repos = subprocess.check_output([list_cmd, REPO_PATH], text=True).strip().split('\n')
    # for d in $HOME/repos-geocom/*/ ; do basename "$d"; done
