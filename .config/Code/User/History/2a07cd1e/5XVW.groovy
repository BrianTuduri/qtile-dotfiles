...
// Método para explorar y validar el mapa de forma recursiva
def checkMap(Map item, List ignoreList, List path = []) {
    item.each { key, value ->
        // Construir el path actual de la clave
        def currentPath = path + [key]
        if (value instanceof Map) {
            // Si el valor es otro mapa, llamada recursiva
            checkMap(value, ignoreList, currentPath)
        } else {
...
// Función principal para validar si un mapa tiene valores vacíos y aceptar excepciones
def validateConfig(Map config, List ignoreList = []) {
    checkMap(config, ignoreList)
}
