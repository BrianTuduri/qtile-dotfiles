    ...
    stages {
        stage("Set settings . . .") {
            steps {
                script {
                    switch(params.instanceEnv) {
                        case "GAMA":
                            gitUrl = [
                                ...
                                "kafka-deploy-repo" : "git@gitlab-gama.geocom.com.uy:uy-com-geocom-scm/kafka-deploy-gama.git",
                                "environments-configuration" : "git@gitlab-gama.geocom.com.uy:uy-com-geocom-scm/gama-environments-configuration.git"
                            ]
                    ...
                    requirmentsYmlData = [ // Create the data structure for YAML
                        collections: [
                            [
        ...
        stage("Set files . . .") {
            steps {
                script {
                    dir("client") {
                        writeFile file: 'ansible/requirements.yml', text: writeYaml(requirmentsYmlData)
                        sh "rm -f .gitmodules" // remove deafult .gitmodules
                ...
                script {
                    sshagent([sshCreds]) {
                        dir('client') {
                            ...
                            sh "ls -lah"
                            sh "cat .gitmodules"
                            sh "git submodule init && git submodule update --remote --merge"
                            dir('Environments') {
                                sh """
        ...
        stage('Get plugins . . .') {
            when { expression { return params.accion != "delete" && params.FILE_URLS != "" } }
            steps {
                script {
                    dir('client') {
                        try {
            ...
            steps {
                script {
                    dir('client') {
                        ...
                        sshagent([sshCreds]) { sh "cat ansible/requirements.yml && ansible-galaxy install -r ansible/requirements.yml" }
