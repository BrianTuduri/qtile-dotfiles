                        ...
                        //sshagent(['cy5ymCych3DLs29tpAnCC2qF0ze5RdPT']) { // root (Clave privada para ambientes QA)
                            sshagent([sshCreds]) { // jenkins (Clave pare construcciones GitLab)
                                configFileProvider([configFile(fileId: 'ansible_agent_cfg', targetLocation: "ansible.cfg")]) {
                                    ...
                                    withCredentials([usernamePassword(credentialsId: 'cred_central_gama_apps', passwordVariable: 'ansible_ssh_pass', usernameVariable: 'ansible_ssh_user')]) {
                                        ansiblePlaybook(
                                            ...
                                            extraVars: [
                                                ...
                                                ansible_user: env.ansible_ssh_user,
                                                ansible_ssh_pass: env.ansible_ssh_pass
                                            ],
                                            tags: "${ansible_tags}",
                                            extras: "-e '@ansible.vars.yml' -v",
                                            colorized: true,
