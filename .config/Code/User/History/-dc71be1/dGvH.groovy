pipeline {
    ...
    stages {
        stage("Set settings") {
            steps {
                if (params.instanceDeploy == 'true') { 

                }
                switch(params.instanceDeploy) {
                    case "GAMA":
                ...
                def requirmentsYml = """
                collections:
                  - name: git@gitlab.geocom.com.uy:scm/geoscm-ansible-collection.git
                    type: git
                    version: master
                  - name: git@gitlab.geocom.com.uy:scm/DevOPS/kubernetes-ansible-collection.git
                    type: git
                    version: master
                """
                script {
                    writeFile(file: 'requirements.yml', text: "${params.ambiente}")
                }
            }
        }
        stage('Ansible . . .') {
