...
def find_or_create_group(gl, path):
    elements = path.split('/')
    project_name = elements.pop()  # Remove and save the last element as it is the project name
    parent_id = None
    ...
    for index, element in enumerate(elements):
        ...
        try:
            ...
            logging.info(f'{"Subgroup" if index else "Group"} "{element}" created.')
        else:
            if found_group:
                group = group
                logging.info(f'{"Subgroup" if index else "Group"} "{element}" already exists.')
        parent_id = group.id  # Update parent_id to nest next group if necessary

    return parent_id, project_name  # Return the ID of the last group and the project name
