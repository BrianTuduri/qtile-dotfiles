...
resource "kubernetes_secret" "ldap_auth" {
  ...
  data = {
    ...
    "SPRING_LDAP_BASE"                   = "DC=geocom,DC=com,DC=uy"
    "SPRING_LDAP_USER_DN_PATTERN"        = "sAMAccountName={0}"
    "SPRING_LDAP_USER_SEARCH_FILTER"     = "&amp;(objectclass=person)(|(uid={0})(sAMAccountName={0}))"
    "SPRING_LDAP_USER_ID_ATTRIBUTE"      = "cn"
    "SPRING_LDAP_AUTHENTICATION_TYPE"    = "simple"
    "LOGGING_LEVEL_ROOT" = "debug"
    SPRING_LDAP_BASE=DC=caca,DC=com,DC=uy
    SPRING_LDAP_USERNAME=gitlabsp@caca.com.uy
    SPRING_LDAP_PASSWORD=myPassword
    SPRING_LDAP_USER_SEARCH_BASE=OU=Uruguay,OU=Caca
    SPRING_LDAP_USER_SEARCH_FILTER=(sAMAccountName={0})
    SPRING_LDAP_GROUP_SEARCH_BASE=
