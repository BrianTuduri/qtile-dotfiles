                    ...
                    dir(CONFIG["CHECKOUT_FOLDER"]) {
                        configFileProvider([configFile(fileId: 'mvn_local_docker_release', targetLocation: "${env.HOME}/.m2/settings.xml")]) {
                            withDockerEnvironment(CONFIG) {
                                ...
                                shDocker "-Dscm.connection= scm:git:https://gitlab.geocom.com.uy/uy-com-geocom-alkosto/geopos2-alkosto.git"
                                shDocker "ls -lah ./ && cd ${projectName} && git checkout ${localCheckout} && git remote rename origin old-origin && git remote add origin ${gitlabUrlOauth2} && mvn ${mavenCommands}"
