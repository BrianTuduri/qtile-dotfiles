    ...
    stages {
        ...
        stage('Ejecucion Maven . . .') {
            steps {
                script {
                    dir(CONFIG["CHECKOUT_FOLDER"]) {
                        def CURRENT_BUILD_VOLUME = "/tmp/builds/:/project/"
                        // /tmp/builds : /tmp/
                        configFileProvider([configFile(fileId: 'mvn_local_docker_release', targetLocation: "${env.HOME}/.m2/settings.xml")]) {
                            ...
                            withDockerEnvironment(CONFIG, 
                                { 
                                    ...
                                    tmpBuildVolume = "/tmp/${UUID.randomUUID()}"
                                ...
                                { containerId ->
                                    ...
                                    shDocker "git clone ${URL_REPO} ${tmpBuildVolume}/${projectName} && ls -lah ${tmpBuildVolume}"
                                    shDocker "cd ${tmpBuildVolume}/${projectName} && git checkout ${localCheckout} && mvn ${mavenCommands} ${mavenArgs} && sleep 5000"
                                }
                            )
                        }
                    }
