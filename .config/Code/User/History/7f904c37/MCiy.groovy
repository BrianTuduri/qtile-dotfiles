                        ...
                        configFileProvider([configFile(fileId: 'mvn_local_docker_release', targetLocation: "${env.HOME}/.m2/settings.xml")]) {
                            ...
                            withDockerEnvironment(CONFIG, 
                                ...
                                { containerId ->
                                    ...
                                    sh "docker ps && docker cp ${sshVolume}/id_rsa ${containerId}:/root/.ssh/id_rsa"
                                    ...
                                    shDocker "cd ${tmpBuildVolume}/${projectName} && git checkout ${localCheckout} && mvn ${mavenCommands} ${mavenArgs} && chown -R 1000:1000 ${tmpBuildVolume}"
                                    tmpJenkinsVolume = tmpBuildVolume.replaceAll("/project/", "/tmp/builds/")+"/${projectName}"
                                    dir (tmpJenkinsVolume) { sh "ls -lah && cat pom.xml"}
                                }
                            )
                        }
                    }
