                    ...
                    dir(CONFIG["CHECKOUT_FOLDER"]) {
                        configFileProvider([configFile(fileId: 'mvn_local_docker_release', targetLocation: "${env.HOME}/.m2/settings.xml")]) {
                            withDockerEnvironment(CONFIG) {
                                def gitlabUrlOauth2
                                def mavenArgs
                                containerId = runDockerContainer(DOCKER_IMAGE, "-u 0 -v ${MVN_VOLUME} --add-host 'gitlab.geocom.com.uy:172.24.18.194'")
                                ...
                                withCredentials([sshUserPrivateKey(credentialsId: '15940393-32ad-416e-ad80-b8ea71536641', keyFileVariable: 'file', usernameVariable: 'user')]) {
                                    shDocker "git config --global user.email geoscm@geocom.com.uy && git config --global user.name 'jenkins'"
                                    shDocker "mkdir /root/.ssh/ && echo $file >> /root/.ssh/id_rsa"
