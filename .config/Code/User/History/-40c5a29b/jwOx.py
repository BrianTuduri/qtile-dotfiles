...
keys = [Key(key[0], key[1], *key[2:]) for key in [
    ...
    # Screenshot
    # # # full screen
    Key([mod], "s", lazy.spawn("sh -c 'FILENAME=~/Pictures/screenshots/$(date \"+%Y-%m-%d_%H-%M-%S\").png; scrot $FILENAME && xclip -selection clipboard -t image/png -i $FILENAME'")),
    # # # selection
    Key([mod, "shift"], "s", lazy.spawn("sh -c 'FILENAME=~/Pictures/screenshots/$(date \"+%Y-%m-%d_%H-%M-%S\")_select.png; scrot -s $FILENAME && xclip -selection clipboard -t image/png -i $FILENAME'")),
]
    # open vscode repos
